...
import { applyResultBackground, resultsBackgroundSrc } from "../storage/premium";
...
export function ResultsSheet({ open, match, onClose }: ResultsSheetProps) {
  ...
  const recap = applyResultBackground(match, resultBackground);
    ...
    >
      ...
      <div className={styles.sheet}>
        <article
          ...
          style={{
            ["--results-home-score-color" as string]: recap.homeColor,
            ["--results-away-score-color" as string]: recap.awayColor,
            ["--results-background-image" as string]: `url("${resultsBackgroundSrc(recap.resultBackground)}")`
          }}
        >
          <button className={styles.close} type="button" aria-label="Close results" onClick={onClose}>
            ×
          </button>
          <time className={styles.date}>{formatMatchDate(recap.updatedAtMs)}</time>
          <div className={styles.topLogo}>
            <LogoMark className={styles.mainLogo} name={recap.homeName} logo={recap.homeLogo} />
          </div>
          <div className={styles.titleBlock}>
            <span>Match Result</span>
            <h3 id="results-title">{recap.title}</h3>
          </div>
          <div className={styles.matchup}>
            <ResultsSide
              name={recap.homeName}
              logo={recap.homeLogo}
              winner={recap.winnerSide === "home"}
            />
            <span className={styles.vs}>VS</span>
            <ResultsSide
              name={recap.awayName}
              logo={recap.awayLogo}
              winner={recap.winnerSide === "away"}
            />
          </div>
