...
export default function DateField({ name, value, defaultValue, onChange, required, ariaLabel }: DateFieldProps) {
  ...
  return (
    <div className={`sf-date ${open ? 'is-open' : ''}`} ref={rootRef}>
      <input name={name} required={required} type="hidden" value={selectedValue} />
      <button aria-expanded={open} aria-haspopup="dialog" aria-label={ariaLabel} className="sf-date-trigger" onClick={() => setOpen((current) => !current)} type="button">
        <span>{display}</span>
        <svg aria-hidden="true" viewBox="0 0 20 20"><path d="M5 2v3M15 2v3M3 7h14M4 4h12a1 1 0 0 1 1 1v12H3V5a1 1 0 0 1 1-1Z" /></svg>
      </button>
      {open ? (
        <div className="sf-popover sf-calendar" role="dialog" aria-label="Choose date">
          ...
          <div className="sf-calendar-footer">
            <button onClick={() => commit(new Date())} type="button">Today</button>
            {selectedValue ? <button onClick={() => { if (value === undefined) setInternalValue(''); onChange?.(''); setOpen(false); }} type="button">Clear</button> : null}
          </div>
        </div>
      ) : null}
    </div>
