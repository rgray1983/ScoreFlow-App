    ...
    <div className="team-workspace">
      <section className="team-summary panel">
        <div>
          ...
          <h2>Organizations, teams & seasons</h2>
          <p>Build the structure that future rosters, schedules, matches, rotations, and reports will use.</p>
        </div>
      ...
      <div className="structure-grid">
        <StructureColumn
          title="Organizations"
          subtitle="School, club, or independent program"
          actionLabel="Add program"
          onAdd={() => setFormMode('organization')}
        >
          {organizations.length === 0 ? <EmptyState label="No programs yet" /> : organizations.map((organization) => (
            <button
              className={`structure-card${workspace.activeOrganizationId === organization.id ? ' is-selected' : ''}`}
              key={organization.id}
              onClick={() => workspace.setActiveOrganization(organization.id)}
              type="button"
            >
              <span className="structure-mark">{organization.name.slice(0, 2).toUpperCase()}</span>
              <span className="structure-copy">
                <strong>{organization.name}</strong>
                <small>{organization.type} · {organization.city}, {organization.state}</small>
              </span>
        ...
        <StructureColumn
          title="Teams"
          subtitle={workspace.activeOrganization?.name ?? 'Select a program first'}
          actionLabel="Add team"
          disabled={!workspace.activeOrganizationId}
          onAdd={() => setFormMode('team')}
        >
          {!workspace.activeOrganizationId ? <EmptyState label="Select a program" /> : teams.length === 0 ? <EmptyState label="No teams in this program" /> : teams.map((team) => (
            <button
              className={`structure-card team-identity${workspace.activeTeamId === team.id ? ' is-selected' : ''}`}
              key={team.id}
              onClick={() => workspace.setActiveTeam(team.id)}
            ...
            >
              ...
              <span className="structure-copy">
                ...
                <small>{team.level} · {team.homeCourt || 'Home court not set'}</small>
              </span>
              <span className="selection-dot" aria-hidden="true" />
            </button>
          ))}
        ...
        <StructureColumn
          title="Seasons"
          subtitle={workspace.activeTeam?.name ?? 'Select a team first'}
          actionLabel="Add season"
          disabled={!workspace.activeTeamId}
          onAdd={() => setFormMode('season')}
        >
          {!workspace.activeTeamId ? <EmptyState label="Select a team" /> : seasons.length === 0 ? <EmptyState label="No seasons for this team" /> : seasons.map((season) => (
            <button
              className={`structure-card${workspace.activeSeasonId === season.id ? ' is-selected' : ''}`}
              key={season.id}
              onClick={() => workspace.setActiveSeason(season.id)}
              type="button"
            >
              <span className={`season-status status-${season.status}`}>{season.status}</span>
              <span className="structure-copy">
                <strong>{season.name}</strong>
                <small>{formatDate(season.startDate)} – {formatDate(season.endDate)}</small>
              </span>
              <span className="selection-dot" aria-hidden="true" />
            </button>
          ))}
      ...
      <section className="active-context panel">
        ...
        <div className="context-path">
          <span>{workspace.activeOrganization?.name ?? 'Program'}</span>
          <b>›</b>
          <span>{workspace.activeTeam?.level ?? 'Team'}</span>
          <b>›</b>
          <span>{workspace.activeSeason?.name ?? 'Season'}</span>
        </div>
...
function StructureColumn({ title, subtitle, actionLabel, onAdd, disabled = false, children }: {
  title: string;
  subtitle: string;
  actionLabel: string;
  onAdd: () => void;
  ...
  return (
    <section className="structure-column panel">
      <header>
        <div><p className="eyebrow">{subtitle}</p><h3>{title}</h3></div>
        <button className="add-structure-button" disabled={disabled} onClick={onAdd} type="button">＋<span>{actionLabel}</span></button>
      </header>
      <div className="structure-list">{children}</div>
    </section>
  );
}
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (mode === 'team') {
      workspace.addTeam({
        ...
        abbreviation: String(data.get('abbreviation') ?? '').trim().toUpperCase().slice(0, 4),
        primaryColor: String(data.get('primaryColor') ?? '#ef3340'),
        secondaryColor: String(data.get('secondaryColor') ?? '#f4c95d'),
        homeCourt: String(data.get('homeCourt') ?? '').trim()
      });
    }
    ...
    if (mode === 'season') {
      workspace.addSeason({
        ...
        status: String(data.get('status')) as SeasonStatus
      });
    }
...
function OrganizationFields() {
  return <div className="form-grid">
    <label className="field-wide"><span>Program name</span><input name="name" required placeholder="Hartsville High School" /></label>
    <label><span>Program type</span><select name="type" defaultValue="school">{organizationTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></label>
    <label><span>City</span><input name="city" required placeholder="Hartsville" /></label>
    <label><span>State</span><input name="state" required maxLength={2} placeholder="SC" /></label>
  </div>;
}

function TeamFields() {
  return <div className="form-grid">
    <label className="field-wide"><span>Team name</span><input name="name" required placeholder="Hartsville Red Foxes" /></label>
    <label><span>Level / age group</span><input name="level" required placeholder="Varsity" /></label>
    <label><span>Abbreviation</span><input name="abbreviation" required maxLength={4} placeholder="HF" /></label>
    <label className="field-wide"><span>Home court</span><input name="homeCourt" placeholder="Main gym" /></label>
    <label><span>Primary color</span><input name="primaryColor" type="color" defaultValue="#ef3340" /></label>
    <label><span>Secondary color</span><input name="secondaryColor" type="color" defaultValue="#f4c95d" /></label>
  </div>;
}

function SeasonFields() {
  return <div className="form-grid">
    <label className="field-wide"><span>Season name</span><input name="name" required placeholder="2026 School Season" /></label>
    ...
    <label className="field-wide"><span>Status</span><select name="status" defaultValue="active">{seasonStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
  </div>;
}
