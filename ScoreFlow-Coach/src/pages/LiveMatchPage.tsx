...
function buildInitial(players:CourtPlayer[],opponent:string):MatchState{const starters=players.filter((player)=>!player.libero).slice(0,6);const libero=players.find((player)=>player.libero);const courtIds=[...starters.map((player)=>player.id),...(starters.length<6&&libero?[libero.id]:[])].slice(0,6);const positions=Object.fromEntries(courtIds.map((id,index)=>[id,defaultPoints[index]]));return{homeScore:0,awayScore:0,set:1,homeSets:0,awaySets:0,serving:'home',rotation:1,courtIds,benchIds:players.map((player)=>player.id).filter((id)=>!courtIds.includes(id)),positions,selectedPlayerId:courtIds[0]||players[0]?.id||'',events:[],homeTimeouts:0,awayTimeouts:0,opponent,started:true}}
