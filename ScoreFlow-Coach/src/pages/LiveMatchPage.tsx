...
type MatchState = { homeScore:number; awayScore:number; set:number; homeSets:number; awaySets:number; serving:Side; rotation:number; courtIds:string[]; benchIds:string[]; selectedPlayerId:string; events:LiveEvent[]; homeTimeouts:number; awayTimeouts:number; opponent:string; started:boolean };

type CourtPlayer = { id:string; name:string; number:string; position:string; libero:boolean };
const STORAGE_KEY='scoreflow-live-match-v1';
...
export default function LiveMatchPage(){
  ...
  const initial=useMemo<MatchState>(()=>readState(players,scheduled?.opponent||'Opponent'),[players,scheduled?.opponent]);
  ...
  const selected=players.find((p)=>p.id===match.selectedPlayerId);
  ...
  function recordStat(action:StatAction){if(!selected)return;addEvent('stat',`#${selected.number} ${selected.name} · ${action}`);setFlash(`${selected.name}: ${action}`);setTimeout(()=>setFlash(''),900)}
  function rotate(){setMatch((current)=>({...current,rotation:current.rotation===6?1:current.rotation+1,courtIds:current.courtIds.length?[current.courtIds[current.courtIds.length-1],...current.courtIds.slice(0,-1)]:current.courtIds}));addEvent('rotation',`Rotated to R${match.rotation===6?1:match.rotation+1}`)}
  function substitute(inId:string){if(!subOut||!inId)return;setMatch((current)=>({...current,courtIds:current.courtIds.map((id)=>id===subOut?inId:id),benchIds:[...current.benchIds.filter((id)=>id!==inId),subOut],selectedPlayerId:inId}));const outgoing=players.find((p)=>p.id===subOut);const incoming=players.find((p)=>p.id===inId);addEvent('sub',`#${incoming?.number} ${incoming?.name} in · #${outgoing?.number} ${outgoing?.name} out`);setSubOut('')}
  ...
  function newMatch(){localStorage.removeItem(STORAGE_KEY);setMatch(buildInitial(players,scheduled?.opponent||'Opponent'))}
  ...
  return <div className="live-match-mode" style={style}>
    <header className="live-scorebar">
      ...
      <div className="live-team-score home"><span>{workspace.activeTeam?.abbreviation||'HOME'}</span><strong>{match.homeScore}</strong><div><button onClick={()=>score('home',-1)} type="button">−</button><button onClick={()=>score('home',1)} type="button">+1</button></div></div>
      ...
      <div className="live-team-score away"><span>{match.opponent}</span><strong>{match.awayScore}</strong><div><button onClick={()=>score('away',-1)} type="button">−</button><button onClick={()=>score('away',1)} type="button">+1</button></div></div>
    ...
    <main className="live-stage">
      <section className="live-court-wrap">
        <div className="live-court">
          ...
          {court.slice(0,6).map((player,index)=><button className={`court-player court-pos-${index+1}${match.selectedPlayerId===player.id?' is-selected':''}${player.libero?' is-libero':''}`} key={player.id} onClick={()=>setMatch((c)=>({...c,selectedPlayerId:player.id}))} type="button"><b>#{player.number}</b><strong>{player.name.split(' ')[0]}</strong><small>{player.position}</small></button>)}
          {court.length<6&&Array.from({length:6-court.length},(_,index)=><div className={`court-player court-pos-${court.length+index+1} is-empty`} key={index}>Open</div>)}
        </div>
        <div className="live-bench"><span>Bench</span>{bench.map((player)=><button className={match.selectedPlayerId===player.id?'is-selected':''} key={player.id} onClick={()=>setMatch((c)=>({...c,selectedPlayerId:player.id}))} type="button"><b>#{player.number}</b><small>{player.name.split(' ')[0]}</small></button>)}</div>
      </section>

      <aside className="stat-console">
        <div className="stat-selected"><span>Two-touch stats</span><strong>{selected?`#${selected.number} ${selected.name}`:'Tap a player'}</strong><small>Score and stats stay independent.</small></div>
        <div className="stat-action-grid">{statActions.map((action)=><button disabled={!selected} key={action} onClick={()=>recordStat(action)} type="button">{action}</button>)}</div>
        <div className="live-tools">
          <button onClick={rotate} type="button">↻ Rotate</button>
          <button onClick={()=>{setMatch((c)=>({...c,homeTimeouts:c.homeTimeouts+1}));addEvent('timeout','Home timeout')}} type="button">Timeout {match.homeTimeouts}</button>
          <button onClick={undo} type="button">↶ Undo event</button>
          <button onClick={()=>setShowTimeline(true)} type="button">☷ Timeline</button>
        </div>
        <div className="sub-console"><span>Substitution</span><div><select value={subOut} onChange={(e)=>setSubOut(e.target.value)}><option value="">Player out</option>{court.map((p)=><option key={p.id} value={p.id}>#{p.number} {p.name}</option>)}</select><select onChange={(e)=>substitute(e.target.value)} value=""><option value="">Player in</option>{bench.map((p)=><option key={p.id} value={p.id}>#{p.number} {p.name}</option>)}</select></div></div>
      </aside>
    </main>
...
function buildInitial(players:CourtPlayer[],opponent:string):MatchState{const starters=players.filter((p)=>!p.libero).slice(0,6);const libero=players.find((p)=>p.libero);const courtIds=[...starters.map((p)=>p.id),...(starters.length<6&&libero?[libero.id]:[])].slice(0,6);return{homeScore:0,awayScore:0,set:1,homeSets:0,awaySets:0,serving:'home',rotation:1,courtIds,benchIds:players.map((p)=>p.id).filter((id)=>!courtIds.includes(id)),selectedPlayerId:courtIds[0]||players[0]?.id||'',events:[],homeTimeouts:0,awayTimeouts:0,opponent,started:true}}
function readState(players:CourtPlayer[],opponent:string){try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null') as MatchState|null;if(!saved)return buildInitial(players,opponent);const ids=new Set(players.map((p)=>p.id));const courtIds=saved.courtIds.filter((id)=>ids.has(id));const known=new Set(courtIds);const benchIds=[...saved.benchIds.filter((id)=>ids.has(id)&&!known.has(id)),...players.map((p)=>p.id).filter((id)=>!known.has(id)&&!saved.benchIds.includes(id))];return{...saved,opponent:saved.opponent||opponent,courtIds,benchIds,selectedPlayerId:ids.has(saved.selectedPlayerId)?saved.selectedPlayerId:(courtIds[0]||benchIds[0]||'')}}catch{return buildInitial(players,opponent)}}