...
export type Player = { id:string; organizationId:string; firstName:string; lastName:string; preferredName:string; photoUrl:string; graduationYear:string; height:string; dominantHand:DominantHand; primaryPosition:string; secondaryPosition:string; notes:string; archived:boolean; createdAt:string };
...
export type MatchSetRecord = { number:number; homeScore:number; awayScore:number; winner:MatchSide | ''; completedAt:string };
...
export type WorkspaceData = { organizations:Organization[]; teams:Team[]; seasons:Season[]; players:Player[]; rosterMemberships:RosterMembership[]; scheduleEvents:ScheduleEvent[]; matches:MatchRecord[]; activeOrganizationId:string; activeTeamId:string; activeSeasonId:string };
...
export type PlayerInput = Pick<Player,'organizationId'|'firstName'|'lastName'|'preferredName'|'photoUrl'|'graduationYear'|'height'|'dominantHand'|'primaryPosition'|'secondaryPosition'|'notes'>;
...
export type MatchInput = Pick<MatchRecord,'scheduleEventId'|'teamId'|'seasonId'|'opponent'|'locationType'|'format'|'regularSetTarget'|'decidingSetTarget'|'winByTwo'|'initialServe'|'startingPlayerIds'|'liberoPlayerId'>;