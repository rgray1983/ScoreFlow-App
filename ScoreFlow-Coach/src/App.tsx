...
function HomePage() {
  ...
  return <div className="dashboard-grid">
    <section className="hero-card panel"><div className="hero-copy"><div className="status-line"><span className="live-dot" /><span>Coach workspace · {activeSeason?.name ?? 'Season not selected'}</span></div><p className="eyebrow">{teamLabel}</p><h2>{nextEvent ? nextEvent.title : activeTeam ? 'Ready for the next point.' : 'Build your volleyball program.'}</h2><p className="hero-summary">{nextEvent ? `${formatDate(nextEvent.date)}${nextEvent.startTime ? ` at ${formatTime(nextEvent.startTime)}` : ''} · ${nextEvent.venue || 'Location not set'}` : activeTeam ? `Working inside ${activeOrganization?.name ?? 'your program'}. Schedules, rosters, matches, and reports use this active team and season.` : 'Create an organization, team, and season to establish the shared context for every Coach tool.'}</p><div className="hero-actions"><NavLink className="button button-primary" to="/schedule">{nextEvent ? 'View schedule' : 'Add schedule event'}</NavLink><NavLink className="button button-quiet" to="/teams">Manage program</NavLink></div></div><div className="match-preview" aria-label="Active team preview"><span className="card-kicker">Active context</span><div className="match-teams"><div><span className="team-badge" style={{ background: activeTeam ? `linear-gradient(145deg, ${activeTeam.primaryColor}, #6f101c)` : undefined }}>{initials}</span><strong>{activeTeam?.name ?? 'Your team'}</strong></div><span className="versus">›</span><div><span className="team-badge opponent">{upcomingEvents.length || '—'}</span><strong>{upcomingEvents.length ? `${upcomingEvents.length} upcoming` : 'Build schedule'}</strong></div></div><div className="match-meta"><span>{activeOrganization?.type ?? 'Program'}</span><strong>{activeTeam?.level ?? 'Team'}</strong><span>{rosterCount} players</span></div></div></section>
    ...
    <section className="insight-panel panel"><div className="panel-heading compact"><div><p className="eyebrow">Program</p><h3>Current structure</h3></div><NavLink className="text-link" to="/players">Players</NavLink></div><div className="metric-grid"><div><span>Program</span><strong>{activeOrganization ? '1' : '0'}</strong></div><div><span>Team</span><strong>{activeTeam ? '1' : '0'}</strong></div><div><span>Roster</span><strong>{rosterCount}</strong></div><div><span>Events</span><strong className="success-text">{upcomingEvents.length}</strong></div></div><div className="insight-note"><span>↗</span><p><strong>One connected workflow.</strong> Scheduled matches will carry team, season, opponent, venue, and timing into Live Match.</p></div></section>
  </div>;
}
...
function App() {
  const [isMenuOpen, setIsMenuOpen] = useState(false); const location = useLocation();
