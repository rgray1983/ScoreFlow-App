...
const quickActions = [
  ...
  { label: 'Players', detail: 'Manage player profiles', path: '/players', tone: 'green' }
] as const;
...
function HomePage() {
  ...
  return <div className="dashboard-grid">
    ...
    <section className="insight-panel panel"><div className="panel-heading compact"><div><p className="eyebrow">Team HQ</p><h3>Match readiness</h3></div><NavLink className="text-link" to="/rosters">Open</NavLink></div><div className="metric-grid"><div><span>Players</span><strong>{roster.length}</strong></div><div><span>Available</span><strong className="success-text">{available}</strong></div><div><span>Starters</span><strong>{starters}</strong></div><div><span>Out</span><strong>{roster.filter((item) => ['injured', 'away', 'inactive'].includes(item.status)).length}</strong></div></div><div className="insight-note"><span>↗</span><p><strong>Players are profiles. Roster is preparation.</strong> Team HQ controls lineup, availability, and performance context.</p></div></section>
  </div>;
}
