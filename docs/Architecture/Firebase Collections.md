...
Fields include public scoreboard state (`homeScore`, `awayScore`, set counts, names, colors, `servingSide`, timeout lights, timeout clock, logo **URLs**, `ended`) plus `ownerId` for the creating scorer. Live games may also store `scorerName` and `scorerAvatar` so a later viewer badge can show who’s scoring. `servingSide` is `"home"`, `"away"`, or empty when the next set still needs a first-serve tap. Timeout fields are `homeTimeouts` / `awayTimeouts` (0–2 per set), `activeTimeout` (`"home"`, `"away"`, or empty), and `timeoutEndsAtMs` (epoch ms when the 45s clock hits 0, or 0 when none). Game IDs are unguessable 128-bit hex strings. Logo and avatar bytes are stored in Firebase Storage under `volleyballGames/{gameId}/` or `users/{userId}/`, not as huge data URLs in the game document.
